...
class TimelineWidget(QGraphicsView):
    ...
    def add_images(self, images):
        ...
        for image in images:
            duration = getattr(image, 'duration', 3)
            ...
            # 添加标签
            text = QGraphicsTextItem(getattr(image, 'name', '图片')[:20])
    ...
    def add_audios(self, audios):
        ...
        for audio in audios:
            duration = getattr(audio, 'duration', 5)
            width = duration * 20  # 每1秒对应20像素宽度
            height = 40
            ...
            # 添加标签
            text = QGraphicsTextItem(getattr(audio, 'name', '音频')[:20])
            text.setFont(QFont("Arial", 8))
            text.setDefaultTextColor(Qt.white)
            text.setPos(position + 5, y_pos + 5)
            self.scene.addItem(text)
