...
class MainWindow(QMainWindow):
    ...
    def create_toolbar(self):
        ...
        # 添加图片按钮
        add_images_action = QAction(QIcon("resources/icons/image.png"), "添加图片", self)
        add_images_action.triggered.connect(self.add_images)
        toolbar.addAction(add_images_action)
        
        # 添加文件夹按钮
        add_folder_action = QAction(QIcon("resources/icons/folder.png"), "添加文件夹", self)
        ...
        # 添加音频按钮
        add_audio_action = QAction(QIcon("resources/icons/audio.png"), "添加音频", self)
        ...
        # 导出视频按钮
        export_action = QAction(QIcon("resources/icons/export.png"), "导出视频", self)
        export_action.triggered.connect(self.export_video)
        toolbar.addAction(export_action)
        
        # 清空按钮
        clear_action = QAction(QIcon("resources/icons/clear.png"), "清空所有", self)
    ...
    def create_sort_combo(self):
        """创建排序下拉框"""
        combo = QComboBox()
        combo.addItems(["按创建时间", "按文件名", "按文件大小"])
        combo.currentIndexChanged.connect(self.sort_image_items)
        return combo
    ...
    def add_folder(self):
        ...
        if folder:
            files = self.media_manager.scan_folder_for_images(folder)
            if files:
                self.media_manager.add_images(files)
                self.status_label.setText(f"添加了 {len(files)} 个图片文件")
